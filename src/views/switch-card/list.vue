<template>
    <div class="app-container">
      ...
      <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
        ...
        <!-- 转出用户名称 -->
        <el-table-column width="110px" align="center" label="转出用户名称">
          <template slot-scope="scope">
            <span>{{scope.row.from_name}}</span>
          </template>
        </el-table-column>
        <!-- 转出用户类型 -->
        <el-table-column width="110px" align="center" label="转出用户类型">
          <template slot-scope="scope">
            <span>{{scope.row.from_type == 1 ? '代理人' : '管理员'}}</span>
          </template>
        </el-table-column>
        <!-- 接收用户名称 -->
        <el-table-column min-width="100px" align="center" label="接收用户名称">
          <template slot-scope="scope">
        ...
        <!-- 接收用户类型 -->
        <el-table-column width="110px" align="center" label="接收用户类型">
          <template slot-scope="scope">
            <span>{{scope.row.to_type == 1 ? '代理人' : '游戏玩家'}}</span>
          </template>
        </el-table-column>
        <!-- 转卡数量 -->
        <el-table-column width="110px" align="center" label="转卡数量">
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.number}}</span>
          </template>
        </el-table-column>
        <!-- 转卡记录时间 -->
        <el-table-column width="160px" align="center" label="转卡记录时间">
          <template slot-scope="scope">
      ...
      <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
        <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
          ...
          <!-- <el-form-item :label="$t('table.number')" prop="number">
            <el-select class="filter-item" v-model="temp.number" placeholder="Please select">
          ...
          <!-- <el-form-item :label="$t('table.price')" prop="price">
            <el-select class="filter-item" v-model="temp.price" placeholder="Please select">
              <el-option v-for="item in  calendarTypeOptions" :key="item.key" :label="item.display_name" :value="item.key">
              </el-option>
            </el-select>
          </el-form-item> -->
  ...
  import { fetchList, createArticle } from '@/api/switch_card'
